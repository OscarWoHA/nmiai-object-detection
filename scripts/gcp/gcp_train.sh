#!/bin/bash
# Run ON the VM. Downloads data directly from GCS, trains everything.
set -e

ZONE=europe-west1-b

echo "=== Installing packages ==="
sudo apt-get update -qq && sudo apt-get install -y -qq unzip libgl1-mesa-glx libglib2.0-0
pip install ultralytics==8.1.0 timm==0.9.12 -q

echo "=== Downloading data from GCS ==="
mkdir -p ~/data && cd ~/data

[ -f coco_dataset.zip ] && echo "coco_dataset.zip already exists, skipping" || curl -L -o coco_dataset.zip 'https://storage.googleapis.com/nmaichamps-participant-data/norgesgruppen-data/NM_NGD_coco_dataset.zip?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=1065880881946-compute%40developer.gserviceaccount.com%2F20260319%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20260319T185007Z&X-Goog-Expires=3600&X-Goog-SignedHeaders=host&X-Goog-Signature='

[ -f product_images.zip ] && echo "product_images.zip already exists, skipping" || curl -L -o product_images.zip 'https://storage.googleapis.com/nmaichamps-participant-data/norgesgruppen-data/NM_NGD_product_images.zip?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=1065880881946-compute%40developer.gserviceaccount.com%2F20260319%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20260319T185007Z&X-Goog-Expires=3600&X-Goog-SignedHeaders=host&X-Goog-Signature='

echo "=== Unpacking ==="
unzip -qo coco_dataset.zip
unzip -qo product_images.zip

echo "=== Converting COCO to YOLO format ==="
python3 << 'PYEOF'
import json, random, os
from pathlib import Path
from collections import Counter, defaultdict

random.seed(42)

DATA = Path(os.path.expanduser("~/data"))

# Find the annotations file
ann_candidates = list(DATA.rglob("annotations.json"))
print(f"Found annotations: {ann_candidates}")
ann_path = ann_candidates[0]

# Find images directory
img_candidates = [d for d in DATA.rglob("images") if d.is_dir() and any(d.glob("*.jpg"))]
print(f"Found image dirs: {img_candidates}")
img_dir = img_candidates[0]

with open(ann_path) as f:
    data = json.load(f)

images = data["images"]
annotations = data["annotations"]
categories = data["categories"]
img_map = {img["id"]: img for img in images}

print(f"Images: {len(images)}, Annotations: {len(annotations)}, Categories: {len(categories)}")

# Stratified split
img_cats = defaultdict(set)
for ann in annotations:
    img_cats[ann["image_id"]].add(ann["category_id"])

cat_counts = Counter(ann["category_id"] for ann in annotations)
rare_cats = {c for c, n in cat_counts.items() if n < 10}
rare_images = {img_id for img_id, cats in img_cats.items() if cats & rare_cats}

all_ids = [img["id"] for img in images]
non_rare = [i for i in all_ids if i not in rare_images]
random.shuffle(non_rare)
val_count = min(max(1, int(len(all_ids) * 0.15)), len(non_rare))
val_ids = set(non_rare[:val_count])
train_ids = set(all_ids) - val_ids
print(f"Train: {len(train_ids)}, Val: {len(val_ids)}")

# Build labels
for nc_mode, nc_name in [("nc1", 1), ("nc356", len(categories))]:
    labels = defaultdict(list)
    for ann in annotations:
        img = img_map[ann["image_id"]]
        x, y, w, h = ann["bbox"]
        cx = max(0, min(1, (x + w/2) / img["width"]))
        cy = max(0, min(1, (y + h/2) / img["height"]))
        nw = max(0, min(1, w / img["width"]))
        nh = max(0, min(1, h / img["height"]))
        cls_id = 0 if nc_mode == "nc1" else ann["category_id"]
        labels[ann["image_id"]].append(f"{cls_id} {cx:.6f} {cy:.6f} {nw:.6f} {nh:.6f}")

    for split, ids in [("train", train_ids), ("val", val_ids)]:
        lbl_dir = DATA / f"labels_{nc_mode}" / split / "labels"
        img_dst = DATA / f"labels_{nc_mode}" / split / "images"
        lbl_dir.mkdir(parents=True, exist_ok=True)
        img_dst.mkdir(parents=True, exist_ok=True)

        for img_id in ids:
            info = img_map[img_id]
            src = img_dir / info["file_name"]
            dst = img_dst / info["file_name"]
            if not dst.exists() and src.exists():
                os.symlink(str(src), str(dst))
            lbl_file = lbl_dir / (Path(info["file_name"]).stem + ".txt")
            lines = labels.get(img_id, [])
            lbl_file.write_text("\n".join(lines) + "\n" if lines else "")

    print(f"Created labels_{nc_mode}")

# Write data.yaml files
cat_names = [c["name"] for c in sorted(categories, key=lambda c: c["id"])]

with open(DATA / "data_nc1.yaml", "w") as f:
    f.write(f"path: {DATA}\ntrain: labels_nc1/train/images\nval: labels_nc1/val/images\n\nnc: 1\nnames: ['product']\n")

with open(DATA / "data_nc356.yaml", "w") as f:
    f.write(f"path: {DATA}\ntrain: labels_nc356/train/images\nval: labels_nc356/val/images\n\nnc: {len(cat_names)}\nnames: {cat_names}\n")

# Build barcode_to_category mapping
meta_candidates = list(DATA.rglob("metadata.json"))
if meta_candidates:
    with open(meta_candidates[0]) as f:
        meta = json.load(f)
    cat_name_to_id = {c["name"]: c["id"] for c in categories}
    mapping = {}
    for product in meta["products"]:
        bc = product["product_code"]
        name = product["product_name"]
        if name in cat_name_to_id:
            mapping[bc] = cat_name_to_id[name]
        else:
            for cn, ci in cat_name_to_id.items():
                if cn.strip().upper() == name.strip().upper():
                    mapping[bc] = ci
                    break
    with open(DATA / "barcode_to_category.json", "w") as f:
        json.dump(mapping, f)
    print(f"Barcode mapping: {len(mapping)} products")

print("Data preparation done!")
PYEOF

export TORCH_FORCE_WEIGHTS_ONLY_LOAD=0

echo "=== Starting nc=1 training ==="
python3 -c "
import torch; _L=torch.load; torch.load=lambda *a,**k: _L(*a,**{**k,'weights_only':False})
from ultralytics import YOLO
model = YOLO('yolov8m.pt')
model.train(
    data='$HOME/data/data_nc1.yaml',
    imgsz=1280, batch=-1, epochs=150, patience=30,
    lr0=0.001, lrf=0.01, cos_lr=True, warmup_epochs=3,
    mosaic=1.0, close_mosaic=10, mixup=0.1,
    hsv_h=0.015, hsv_s=0.4, hsv_v=0.4,
    translate=0.1, scale=0.3, fliplr=0.5, flipud=0.0, degrees=0.0,
    erasing=0.1, weight_decay=0.0005, max_det=500,
    project='$HOME/runs', name='yolov8m_nc1',
    save=True, plots=True, device=0,
)
"

echo "=== Starting nc=356 training ==="
python3 -c "
from ultralytics import YOLO
model = YOLO('yolov8m.pt')
model.train(
    data='$HOME/data/data_nc356.yaml',
    imgsz=1280, batch=-1, epochs=150, patience=30,
    lr0=0.001, lrf=0.01, cos_lr=True, warmup_epochs=3,
    mosaic=1.0, close_mosaic=10, mixup=0.1,
    hsv_h=0.015, hsv_s=0.4, hsv_v=0.4,
    translate=0.1, scale=0.3, fliplr=0.5, flipud=0.0, degrees=0.0,
    erasing=0.1, weight_decay=0.0005, max_det=500,
    project='$HOME/runs', name='yolov8m_nc356',
    save=True, plots=True, device=0,
)
"

echo "=== Computing DINOv2 embeddings ==="
python3 << 'PYEOF'
import json, numpy as np, torch, timm
from timm.data import resolve_data_config, create_transform
from PIL import Image
from pathlib import Path
import os

DATA = Path(os.path.expanduser("~/data"))
RUNS = Path(os.path.expanduser("~/runs"))
RUNS.mkdir(exist_ok=True)

barcode_to_cat = json.loads((DATA / "barcode_to_category.json").read_text())
model = timm.create_model("vit_small_patch14_dinov2.lvd142m", pretrained=True, num_classes=0)
model = model.half().cuda().eval()
data_config = resolve_data_config(model=model)
transform = create_transform(**data_config, is_training=False)

# Save weights
torch.save(model.cpu().state_dict(), str(RUNS / "dinov2_vits14.pt"))
model = model.cuda()
print(f"DINOv2 weights saved: {(RUNS / 'dinov2_vits14.pt').stat().st_size / 1e6:.1f} MB")

# Find multiangle dir
multiangle_candidates = list(DATA.rglob("images_multiangle"))
multiangle = [d for d in multiangle_candidates if d.is_dir()][0]
print(f"Multiangle dir: {multiangle}")

all_emb, all_ids = [], []
for d in sorted(multiangle.iterdir()):
    if not d.is_dir() or d.name == "__MACOSX":
        continue
    bc = d.name
    if bc not in barcode_to_cat:
        continue
    angle_embs = []
    for p in sorted(d.glob("*.jpg")):
        try:
            img = Image.open(p).convert("RGB")
            t = transform(img).unsqueeze(0).cuda().half()
            with torch.no_grad():
                e = model(t)
                e = e / e.norm(dim=-1, keepdim=True)
                angle_embs.append(e.cpu().float().numpy()[0])
        except Exception as ex:
            print(f"Error: {p}: {ex}")
    if angle_embs:
        avg = np.mean(angle_embs, axis=0)
        avg = avg / np.linalg.norm(avg)
        all_emb.append(avg)
        all_ids.append(barcode_to_cat[bc])

torch.save({
    "embeddings": np.array(all_emb, dtype=np.float32),
    "category_ids": np.array(all_ids, dtype=np.int64),
}, str(RUNS / "reference_embeddings.pt"))
print(f"Saved {len(all_emb)} product embeddings, shape: {np.array(all_emb).shape}")
PYEOF

echo ""
echo "=== ALL DONE ==="
echo "Download weights from local machine:"
echo "  gcloud compute scp yolo-train:~/runs/yolov8m_nc1/weights/best.pt ./yolo_nc1_best.pt --zone=$ZONE"
echo "  gcloud compute scp yolo-train:~/runs/yolov8m_nc356/weights/best.pt ./yolo_nc356_best.pt --zone=$ZONE"
echo "  gcloud compute scp yolo-train:~/runs/dinov2_vits14.pt . --zone=$ZONE"
echo "  gcloud compute scp yolo-train:~/runs/reference_embeddings.pt . --zone=$ZONE"
echo ""
echo "Then DELETE the VM:"
echo "  gcloud compute instances delete yolo-train --zone=$ZONE --quiet"

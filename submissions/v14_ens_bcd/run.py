"""v12: 3-model ensemble with sweep-tuned WBF.

Models:
- model_a.pt: YOLOv8l nc=356 (original training)
- model_b.pt: YOLOv8l nc=356 (GPT-recipe training)
- model_c.pt: YOLOv8x nc=356 (different architecture)
"""
import argparse
import json
...
def main():
    ...
    models = [
        (YOLO(str(script_dir / "model_a.pt")), 2.0),   # original best
        (YOLO(str(script_dir / "model_b.pt")), 1.8),   # GPT-recipe
        (YOLO(str(script_dir / "model_c.pt")), 1.0),   # YOLOv8x
    ]
    ...
    for img_path in sorted(Path(args.input).iterdir()):
        ...
        for model, model_weight in models:
            for imgsz, conf, scale_wt in [(1088, 0.015, 0.8), (1280, 0.01, 1.0)]:
                ...
                if boxes:
                    ...
                    weights.append(model_weight * scale_wt)

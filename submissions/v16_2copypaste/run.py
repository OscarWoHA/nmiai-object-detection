...
def main():
    ...
    model_a = YOLO(str(script_dir / "model_a.onnx"))
    model_b = YOLO(str(script_dir / "model_b.onnx"))
    ...
    for img_path in sorted(Path(args.input).iterdir()):
        ...
        # Both models at 1280 (ONNX fixed size)
        for model, model_wt in [(model_a, 1.5), (model_b, 1.5)]:
            boxes, scores, labels = run_model(model, img_path, device, w, h, 1280, 0.01)
            if boxes:

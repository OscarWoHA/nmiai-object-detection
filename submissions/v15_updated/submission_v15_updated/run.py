...
def main():
    ...
    model_a = YOLO(str(script_dir / "model_a.pt"))   # YOLOv8l original
    model_b = YOLO(str(script_dir / "model_b.onnx"))  # YOLOv8l copy-paste (ONNX FP16)
    ...
    for img_path in sorted(Path(args.input).iterdir()):
        ...
        # Model B (ONNX, fixed at imgsz=1280)
        for imgsz, conf, wt in [(1280, 0.01, 2.0)]:
